...
//		return mysql_close ( $this->link_id );
	...
	{
		if(strtolower($_SESSION['check'])!=strtolower($checkcode))
		{
			return 1;
		}
		else
		{
			...
			$sql = "select * from t_users where FUserName='" . $username . "' and FPassWord='" . $password  . "'";
			$row = $this->find($sql);
			if($row==false)
			{
				return 2;
			...
			else
			{
				//成功登录
				session_start();
				$_SESSION["UserName"]=$row["FUName"];
				//$_SESSION["UserType"]=2;
				$_SESSION["Login"]=date("Y-m-d h:i:s");
				//$_SESSION["LoginUserRole"] = $row["FFunctionID"];   //权限
				return 0;
			}
		}
	}
